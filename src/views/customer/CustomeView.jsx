...
import { DataProvider, useData } from "../../context/userContext";
...
const CustomerView = ({ customer }) => {
  ...
  const userTransactions = transactions
    ?.filter(
      (transaction) =>
        transaction.user?._id === customer._id &&
        transaction.status === "approved"
    )
    .sort((a, b) => new Date(a.transactionDate) - new Date(b.transactionDate));

  const statements = userTransactions?.map((transaction) => {
  ...
  const userBalance =
    statements.length > 0 ? statements[statementsSize - 1]["balance"] : 0;
  ...
  return (
    <div className="flex my-4 gap-4">
      ...
      <Portfolio className="w-2/12">
          ...
          <Table>
            <tbody>
              ...
              <TableDataRow>
                ...
                <TableRowCell>{customer?.dseAccount}</TableRowCell>
              </TableDataRow>
              <TableDataRow>
                <TableRowCell>Balance</TableRowCell>
                <TableRowCell>
                  {Intl.NumberFormat().format(userBalance)}
                </TableRowCell>
              </TableDataRow>
              <TableDataRow>
        ...
        <ActionLinks customerId={customer?._id} statements={statements} />
